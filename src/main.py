...
from helpers.early_stopping import early_stopping
...
datapath = '../../data/%s/' % network_info.name
...
# cross_validate(X=X_train, y=y_train, n_folds=5, network_info=network_info)
...
with open(filepath + ('%s_%d_predictions.txt' % (network_info.name, network_info.id)), 'w') as file:
...
print('Network training took %s seconds' % (time.time() - start_time))
# ------------------------------------------------------------------------------------------------------------------
